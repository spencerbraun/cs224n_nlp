...
def naiveSoftmaxLossAndGradient(
    ...
    probdiff = yhat.copy()
    probdiff[outsideWordIdx] -= 1
    gradCenterVec = np.dot(outsideVectors.T, probdiff)
    gradOutsideVecs = np.dot(probdiff[:, None], centerWordVec[None, :])
...
def negSamplingLossAndGradient(
    ...
    outside_sg  = sigmoid(np.dot(outsideVectors[outsideWordIdx], centerWordVec))
    neg_sg = sigmoid(-np.dot(outsideVectors[negSampleWordIndices], centerWordVec))
    loss = -np.log(outside_sg) - np.sum(np.log(neg_sg))

    gradCenterVec = (outside_sg - 1) * outsideVectors[outsideWordIdx]
    gradCenterVec += np.dot((1-neg_sg), outsideVectors[negSampleWordIndices])

    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx, :] += (outside_sg - 1) * centerWordVec
    # import ipdb; ipdb.set_trace()
    # gradOutsideVecs[negSampleWordIndices, :] += np.outer((1 - neg_sg[:,None]), centerWordVec)
    for order, i in enumerate(negSampleWordIndices):
        gradOutsideVecs[i, :] += (1 - neg_sg[order])  * centerWordVec
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        ...
        loss_i, gradCenterVec_i, gradOutsideVecs_i = word2vecLossAndGradient(
            centerWordVec,
            outsideWordIdx,
            outsideVectors,
            dataset
        ...
        loss += loss_i
        gradCenterVecs[centerIdx] += gradCenterVec_i
